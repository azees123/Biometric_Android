...
class FingerprintApp(App):
    ...
    def check_fingerprint(self, fingerprint_image_path, reg_no):
        ...
        # Simulate the verification by comparing the fingerprint image path
    ...
    def open_file_chooser(self, callback):
        ...
        file_chooser.bind(on_selection=lambda x: self.select_image(file_chooser.selection, callback))
        file_chooser_popup = Popup(title="Select Fingerprint Image", content=file_chooser, size_hint=(0.9, 0.9))
        file_chooser_popup.open()

    def select_image(self, selection, callback):
        if selection:
            fingerprint_image_path = selection[0]
            print(f"Fingerprint image selected: {fingerprint_image_path}")
            callback(fingerprint_image_path)
    ...
    def register_user(self, instance):
        ...
        self.aadhaar_no_input = TextInput(hint_text="Enter your Aadhaar number", size_hint=(1, None), height=40)
        ...
        self.popup_register.add_widget(self.aadhaar_no_input)
    ...
    def capture_and_register(self):
        ...
        aadhaar_no = self.aadhaar_no_input.text
        ...
        def after_image_selected(fingerprint_image_path):
            # Capture photo (simulated in this case)
            photo_path = os.path.join(APP_PATH, f"{name}_photo.png")

            if self.save_user_details(name, phone, reg_no, aadhaar_no, photo_path, fingerprint_image_path):
                self.show_popup_message("Success", f"User {name} registered successfully.")

        self.open_file_chooser(after_image_selected)
    ...
    def perform_fingerprint_verification(self, instance):
        ...
        # Ask user to select the fingerprint image for verification
        def after_image_selected(fingerprint_image_path):
