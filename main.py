...
class FingerprintApp(App):
    ...
    def register_user(self, instance):
        ...
        self.dob_spinner = Spinner(text="DD/MM/YYYY", values=("01/01/1990", "02/02/1991", "03/03/1992", "04/04/1993", "05/05/1994"), size_hint=(1, None), height=40)
        ...
        self.gallery_button = Button(text="Select Profile Image", size_hint=(1, None), height=50)
    ...
    def capture_and_register(self, instance):
        ...
        # Save user details
        fingerprint_data = f"fingerprint_{emp_id}_data"  # Placeholder for actual fingerprint data
        photo_path = getattr(self, 'photo_path', None)  # Placeholder photo path from gallery
        ...
        self.save_user_db()
        self.show_popup_message("Success", f"User {name} registered successfully. Timestamp: {user_db[emp_id]['registration_timestamp']}")

    def select_image(self, instance):
        filechooser = FileChooserIconView()
        filechooser.bind(on_selection=lambda *x: self.load_image(filechooser.selection))
        
        image_popup = Popup(title="Select Profile Image", content=filechooser, size_hint=(0.8, 0.8))
        image_popup.open()
    ...
    def load_image(self, selection):
        if selection:
            ...
            self.show_popup_message("Success", f"Image {image_path} selected.")
        else:
    ...
    def verify_fingerprint(self, instance):
        ...
        self.emp_id_verify_input = TextInput(hint_text="Enter employee ID", size_hint=(1, None), height=40)
        self.popup_verify.add_widget(self.emp_id_verify_input)
    ...
    def perform_fingerprint_verification(self, instance):
        emp_id = self.emp_id_verify_input.text
        fingerprint_data = f"fingerprint_{emp_id}_data"  # Placeholder for actual fingerprint data

        if emp_id not in user_db:
            self.show_popup_message("Error", "Employee ID not found in database!")
            return

        if fingerprint_data == user_db[emp_id]['fingerprint']:
            self.show_popup_message("Access Granted", f"Fingerprint verified successfully!\nName: {user_db[emp_id]['name']}\nEmployee ID: {emp_id}\nTimestamp: {user_db[emp_id]['registration_timestamp']}")
        else:
            self.show_popup_message("Access Denied", f"Fingerprint verification failed.\nTimestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
